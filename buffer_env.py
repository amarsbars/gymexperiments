...
class Buffer_env:
    def __init__(self, size, observation_shape, action_shape, observation_dtype=np.float, action_dtype=np.integer, reward_dtype=np.float):
        ...
        self.env = np.empty(self.size, dtype=np.int)
        self.terminals = np.empty(self.size, dtype=np.bool)

        self.count = 0
        self.current = 0

    def add(self, preobs, action, reward, postobs, terminal, env):
    ...
    def sample(self, batch_size):
        ...
        return self.preobs[indexes], self.actions[indexes], self.rewards[indexes], self.postobs[indexes], self.terminals[indexes], self.env[indexes]
