...
def createLayers():
  ...
  v = Dense(args.hidden_size, name='v1', W_constraint=W_constraint, W_regularizer=regularizer())(h)
  v = Dense(1, name='v2', W_constraint=W_constraint, W_regularizer=regularizer())(v)
  m = Dense(args.hidden_size, name='m1', W_constraint=W_constraint, W_regularizer=regularizer())(h)
  m = Dense(num_actuators, name='m2', W_constraint=W_constraint, W_regularizer=regularizer())(m)
...
# get weights for layers to swap
V_weights_mod = list()
M_weights_mod = list()
V_weights_targ = list()
M_weights_targ = list()
# pdb.set_trace()
V_weights_temp = model.layers[10].get_weights()
M_weights_temp = model.layers[4].get_weights()
for i_v in range(len(filenames)):
  V_weights_mod.append(V_weights_temp)
  M_weights_mod.append(M_weights_temp)
  V_weights_targ.append(V_weights_temp)
  M_weights_targ.append(M_weights_temp)
...
for i_episode in xrange(args.episodes):
    ...
    #set layer weights
    model.layers[10].set_weights(V_weights_mod[env_e])
    model.layers[4].set_weights(M_weights_mod[env_e])
    target_model.layers[10].set_weights(V_weights_targ[env_e])
    target_model.layers[4].set_weights(M_weights_targ[env_e])
    ...
    # save moel weights
    V_weights_mod[env_e] = model.layers[10].get_weights()
    M_weights_mod[env_e] = model.layers[4].get_weights()
    V_weights_targ[env_e] = target_model.layers[10].get_weights()
    M_weights_targ[env_e] = target_model.layers[4].get_weights()
...
# save models
for i_save_env in range(len(env_list_names)):
  target_model.layers[10].set_weights(V_weights_targ[env_e])
  target_model.layers[4].set_weights(M_weights_targ[env_e])
  target_model.save(fn + '_' + str(i_save_env) + '.hd5')
